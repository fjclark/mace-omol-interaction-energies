...
class PocketMLPResult:
    ...
    energy_delta_kcal_per_mol: float
    status: str
    error_message: str | None
...
def make_paths(pdb_id: str, output_root: Path) -> SystemPaths:
    ...
    return SystemPaths(
        ...
        best_sdf=results_dir / "best_lowest_water_hof.sdf",
        mlp_minimised_sdf=results_dir / "best_lowest_water_hof_mlp_minimised.sdf",
...
def compare_water_minus_vacuum(paths: SystemPaths) -> dict[str, Any]:
    """Calculate water-minus-vacuum for reporting, but select by lowest water HOF."""
    ...
    rows_for_csv = sorted(rows, key=lambda r: r["water_minus_vacuum"])
    with paths.delta_results_csv.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows_for_csv[0].keys()))
        writer.writeheader()
        writer.writerows(rows_for_csv)

    best = min(rows, key=lambda r: r["water_heat_of_formation"])

    print(
        f"[{paths.pdb_id}] Selected={best['conformer']} "
        f"lowest water HOF={best['water_heat_of_formation']} "
...
def write_best_water_optimised_sdf(paths: SystemPaths, best: dict[str, Any]) -> Path:
    ...
    mol.SetProp("_Name", f"{paths.pdb_id}_{best['conformer']}_lowest_water_hof")
    mol.SetProp("pdb_id", paths.pdb_id)
    mol.SetProp("source_type", "mopac_water_optimised_lowest_water_hof")
...
def write_same_selected_rdkit_conformer_sdf(paths: SystemPaths, best: dict[str, Any]) -> Path:
    ...
    mol.SetProp("selected_conformer", best["conformer"])
    mol.SetProp("selection_source", "lowest_water_heat_of_formation")
    mol.SetProp("water_heat_of_formation", str(best["water_heat_of_formation"]))
    mol.SetProp("water_minus_vacuum", str(best["water_minus_vacuum"]))
...
"""This class creates a OpenMM minimisation reporter that tracks ligand gradients and ligand coordinate movement at each reported optimisation iteration
# It inherits from `MinizationReporter`, which means it can be passed into:
# `simulation.minimizeEnergy(..., reporter=reporter)`"""

class Reporter(MinimizationReporter):
    """Print minimizer progress for debugging OpenMM/OpenMM-ML relaxation."""

    def __init__(self, ligand_atoms: Iterable[int]) -> None:
        super().__init__()
        # Store ligand atom indices
        self.ligand_atoms = np.asarray(
            sorted(set(ligand_atoms)),
            dtype=int,
        )

        # Initialise call counter
        # This stores the number of times OpenMM has called report()
        # Each time report() runs, the code increments it.
        self.calls = 0

        # Initialise reference ligand positions
        # Creates an instance attribute that contain either n-dimension array or None
        # Initially, no reference coordinates have been recorded, so set to None
        self.initial_ligand_positions: np.ndarray | None = None
    ...
    def report(self, iteration, x, grad, args):
        ...
        energy_kj_per_mol = float(args["system energy"])
        energy_kcal_per_mol = energy_kj_per_mol / 4.184
        print(
            f"{iteration} {energy_kj_per_mol:.12f} kJ/mol "
            f"({energy_kcal_per_mol:.12f} kcal/mol)",
            flush=True,
        )
        return False
...
def normalise_complex_ligand_resname(
    input_pdb: Path,
    output_pdb: Path,
    old_resname: str = "UNL",
    new_resname: str = "LIG",
) -> Path:
    """Write a copy of a PDB with ligand residue name UNL changed to LIG.

    Only fixed-width PDB coordinate records whose residue-name field exactly
    matches ``old_resname`` are modified. Coordinates, atom names, serial
    numbers, chain IDs, residue numbers, occupancies, and B-factors are
    preserved.
    """
    if len(old_resname) > 3 or len(new_resname) > 3:
        raise ValueError("PDB residue names must contain at most three characters.")

    input_pdb = Path(input_pdb)
    output_pdb = Path(output_pdb)

    if not input_pdb.exists():
        raise FileNotFoundError(f"Complex PDB not found: {input_pdb}")
    ...
    with input_pdb.open("r", encoding="utf-8", errors="replace", newline="") as handle:
        for line in handle:
            record_name = line[:6]
            if (
                record_name in {"ATOM  ", "HETATM", "ANISOU", "TER   "}
                and len(line) >= 20
                and line[17:20].strip() == old_resname
            ):
    ...
    with output_pdb.open("w", encoding="utf-8", newline="") as handle:
        handle.writelines(output_lines)

    if changed_records:
        print(
            f"Converted {changed_records} PDB records from "
            f"{old_resname} to {new_resname}: {output_pdb}",
            flush=True,
        )
    else:
        print(
            f"No {old_resname} residue records found; wrote normalised copy: "
            f"{output_pdb}",
            flush=True,
        )
...
def find_pocket_residue_atoms(topology: Any, positions: Quantity, ligand_atoms: list[int], cutoff_angstrom: float, include_ligand: bool = False) -> list[int]:
    """Return all atoms belonging to protein residues with min distance < cutoff from any ligand atom."""
    ...
    for residue in topology.residues():
        ...
        # Strictly within 5 Å when cutoff_angstrom is 5.0.
...
def choose_mobile_atoms(
    topology: Any,
    ligand_atoms: list[int],
    pocket_atoms: list[int],
    freeze_backbone: bool,
) -> list[int]:
    """Ligand and residues within 5 Å are flexible; all other atoms are frozen.

    freeze_backbone is kept for CLI compatibility but intentionally ignored here.
    """
    return sorted(set(ligand_atoms) | set(pocket_atoms))
...
def create_openmmml_mixed_system(
    ...
    """Create an OpenMM-ML mixed system.

    In the default CLI configuration, both ligand and residues within 5 Å are in the
    ML region, while the rest remains AMBER14/SMIRNOFF MM and is frozen by zero mass.
    """
...
def run_pocket_mlp_optimisation(
    ...
    print(f"[{pdb_id}] Pocket MLP optimisation", flush=True)
    print(f"[{pdb_id}] Complex input: {complex_pdb.resolve()}", flush=True)
    print(f"[{pdb_id}] Ligand input:  {ligand_sdf.resolve()}", flush=True)

    pdb = PDBFile(str(complex_pdb))
    ligand_molecule = read_ligand_as_openff(ligand_sdf)

    # Match pocket_mlp_minimal.py exactly for the pocket calculation:
    # use the requested model directly and do not switch models by ligand charge.
    # The auto_model_by_charge argument is retained for API/CLI compatibility.
    ...
    # Ligand + 5 Å residue atoms are flexible. Everything else is frozen.
    mobile_atoms = choose_mobile_atoms(pdb.topology, ligand_atoms, pocket_atoms, freeze_backbone)
    
    print(f"[{pdb_id}] Ligand atoms ({len(ligand_atoms)}): {ligand_atoms}", flush=True)
    print(f"[{pdb_id}] Pocket atoms ({len(pocket_atoms)}): {pocket_atoms}", flush=True)
    print(f"[{pdb_id}] Mobile atoms ({len(mobile_atoms)}): {mobile_atoms}", flush=True)
    print(f"[{pdb_id}] ML atoms ({len(ml_atoms)}): {ml_atoms}", flush=True)
    ...
    print(
        f"[{pdb_id}] Minimal-aligned settings: cutoff={cutoff_angstrom:.1f} A, "
        f"pocket_mode={pocket_mode}, max_iterations={max_iterations}, "
        f"platform={platform_name}, nonbonded_mode={nonbonded_mode}",
        flush=True,
    )
    ...
    # Check whether ligand particles have zero mass
    print(
        f"[{pdb_id}] Checking ligand masses:",
        flush=True,
    )

    for atom_index in ligand_atoms:
        mass = system.getParticleMass(atom_index).value_in_unit(
            unit.dalton
        )

        print(
            f" ligand atom={atom_index:6d} "
            f"mass={mass:.6f} Da",
            flush=True,
        )
    ...
    try:
        ...
        initial_positions_nm = np.asarray(
            initial_state.getPositions(asNumpy=True).value_in_unit(
                unit.nanometer
            )
        )

        # Convert initial forces to Numpy
        initial_forces = np.asarray(
            initial_state.getForces(asNumpy=True).value_in_unit(
                unit.kilojoules_per_mole / unit.nanometer
            ),
            dtype=float,
        )
        
        # Convert atom-index lists into Numpy arrays
        ligand_indices = np.asarray(ligand_atoms, dtype=int)
        mobile_indices = np.asarray(mobile_atoms, dtype=int)

        # Calculate ligand force magnitudes
        ligand_initial_force_norms = np.linalg.norm(
            initial_forces[ligand_indices],
            axis=1,
        )

        mobile_initial_force_norms = np.linalg.norm(
            initial_forces[mobile_indices],
            axis=1,
        )

        # Print initial energy
        print(
            f"[{pdb_id}] Initial mix-system energy = "
            f"{initial_energy:.6f} kcal/mol",
            flush=True,
        )

        # Calculate and print ligand-force RMS
        print(
            f"[{pdb_id}] Initial ligand force RMS = "
            f"{np.sqrt(np.mean(ligand_initial_force_norms**2)):.6f} "
            f"KJ/mol/nm",
            flush=True
        )

        # Print maximum ligand force
        print(
            f"[{pdb_id}] Maximum ligand force = "
            f"{np.max(ligand_initial_force_norms):.6f} "
            f"KJ/mol/nm",
            flush=True
        )

        # Print mobile-region force RMS
        # This reports the RMS force magnitude across ligand and pocket atoms together
        # Comparing it with ligand RMS can show where the force is concentrated
        print(
            f"[{pdb_id}] Mobile-regin force RMS= "
            f"{np.sqrt(np.mean(mobile_initial_force_norms**2)):.6f}"
            f"KJ/mol/nm",
            flush=True,
        )
        ...
        print(
            f"[{pdb_id}] Minimisation returned.",
            flush=True
        )

        # Request the final state
        ...
        # Create a unitless Numpy copy
        # This converts the final coordinates into a normal NumPy array in nanometers.
        # This version is used for coordinate subtraction and displacement calculations
        final_positions_nm = np.asarray(
            final_positions.value_in_unit(unit.nanometer),
            dtype=float,
        )

        # Retrieve final forces
        final_force = np.asarray(
            final_state.getForces(asNumpy=True).value_in_unit(
                unit.kilojoules_per_mole / unit.nanometer
            ),
            dtype=float,
        )

        # Calculate ligand displacement vectors
        # converts the coordinates from nanometres to angstroms
        ligand_displacement_vectors_angstrom = (
            final_positions_nm[ligand_indices] - initial_positions_nm[ligand_indices]
        ) * 10.0

        # Calculate ligand displacement magnitudes
        ligand_displacements_angstrom = np.linalg.norm(
            ligand_displacement_vectors_angstrom,
            axis=1,
        )

        # Create pocket index array
        ...
        if pocket_indices.size > 0:
            pocket_displacement_vectors_angstrom = (
                final_positions_nm[pocket_indices]
                - initial_positions_nm[pocket_indices]
            ) * 10.0
            ...
            pocket_rms_displacement = float(
                np.sqrt(np.mean(pocket_displacements_angstrom**2))
            )
            pocket_max_displacement = float(
                np.max(pocket_displacements_angstrom)
            )
        else:
            pocket_rms_displacement = None
            pocket_max_displacement = None

        # Calculate final ligand force magnitudes
        # This calculates the final force-vector magnitude for each ligand atom.
        ligand_final_force_norms = np.linalg.norm(
        ...
        # Print energy change
        print(
            f"[{pdb_id}] Energy change = "
            f"{energy_delta:.10f} kcal/mol",
            flush=True,
        )

        # Calculate ligand RMS displacement
        # This computes the distance moved by ligand atom
        # This is not an alignment-corrected molecular RMSD, it is the direct coordinate displacement in the original coordinate frame
        print(
            f"[{pdb_id}] Ligand RMS displacement = "
            f"{np.sqrt(np.mean(ligand_displacements_angstrom**2)):.6f} A",
            flush=True
        )

        # Print maximum ligand displacement
        # This reports the movement of the single ligand atom that moved the most
        print(
            f"[{pdb_id}] Maximum ligand displacement: = "
            f"{np.max(ligand_displacements_angstrom):.6f} A",
            flush=True,
        )


        # Report pocket movement only when a non-empty pocket was selected.
        if pocket_rms_displacement is not None:
            print(
                f"[{pdb_id}] Pocket RMS displacement = "
                f"{pocket_rms_displacement:.6f} A",
                flush=True,
            )
            print(
                f"[{pdb_id}] Pocket maximum displacement = "
                f"{pocket_max_displacement:.6f} A",
                flush=True,
            )
        else:
            print(
            ...
            print(
                f"[{pdb_id}] Pocket displacement is not applicable.",
                flush=True,
            )

        # Final ligand force RMS
        # This calculates the RMS magnitude of ligand forces after minimisation.
        # Ideally: final ligand force RMS < initial ligand force RMS
        print(
            f"[{pdb_id}] Final ligand force RMS: "
            f"{np.sqrt(np.mean(ligand_final_force_norms**2)):.6f} kJ/mol/nm"
        )
        ...
        return PocketMLPResult(
            ...
            energy_delta_kcal_per_mol=float(energy_delta),
            status="ok",
            error_message=None,
        )
    finally:
...
def run_one_pdb(
    ...
    row: dict[str, Any] = {
        ...
        "selection_criterion": "lowest_water_heat_of_formation",
        "water_heat_of_formation": best["water_heat_of_formation"],
    ...
    if not skip_ligand_mlp:
        mopac_mlp_result = run_mlp_on_sdf(
            pdb_id,
            "mopac_water_optimised_lowest_water_hof",
            mopac_best_sdf,
            model,
            auto_model_by_charge,
            restraint_k,
            max_iterations,
            platform,
            paths.mlp_minimised_sdf,
        )
        write_dataclass_result(paths.mlp_json, paths.mlp_csv, mopac_mlp_result)

        rdkit_mlp_result = run_mlp_on_sdf(
            pdb_id,
            "original_rdkit_same_selected_conformer",
            same_rdkit_sdf,
            model,
            auto_model_by_charge,
            restraint_k,
    ...
    if not skip_pocket_mlp:
        # For pocket MLP, use the PL-REX bound ligand SDF, not the MOPAC-selected ligand.
        bound_ligand_sdf = paths.input_sdf
        ...
        # Normalise PL-REX ligand residue naming before the pocket calculation.
        # This preserves all coordinates and changes only the PDB residue-name
        # field from UNL to LIG.
        complex_path = normalise_complex_ligand_resname(
            input_pdb=complex_path,
            output_pdb=paths.input_dir / "complex_lig.pdb",
            old_resname="UNL",
            new_resname="LIG",
        )
...
def main(
    ...
    model: AvailableModel = typer.Option("mace-off23-small", "--model", help="MLP model. Pocket MLP uses this requested model directly."),
    auto_model_by_charge: bool = typer.Option(True, "--auto-model-by-charge/--no-auto-model-by-charge", help="Use aceff-2.0 for charged ligands in ligand-only MLP stages; the pocket stage uses --model directly to match minimal.py."),
    ...
    protein_pdb: Path | None = typer.Option(None, "--protein-pdb", help="Protein PDB. If supplied, the script combines it with the PL-REX bound ligand SDF."),
    complex_pdb: Path | None = typer.Option(None, "--complex-pdb", help="Pre-built protein-ligand complex PDB. UNL ligand residue records are copied and renamed to LIG before pocket minimisation."),
    ligand_resname: str = typer.Option("LIG", "--ligand-resname", help="Residue name used when constructing a complex from separate protein and ligand files. Pocket minimisation normalises UNL to LIG."),
    pocket_cutoff: float = typer.Option(0.0, "--pocket-cutoff", min=0.0, help="Residue cutoff around ligand in Angstrom. Default 0.0 matches minimal.py and selects no protein pocket atoms."),
    pocket_mode: PocketMode = typer.Option(
        "ligand+pocket",
        "--pocket-mode",
        help="Atoms treated by OpenMM-ML. Default: ligand plus protein residues within cutoff.",
    ),
    freeze_backbone: bool = typer.Option(
        False,
        "--freeze-backbone/--mobile-backbone",
        help="Default False matches minimal.py. This option is retained for compatibility; mobile atoms are ligand plus selected pocket atoms.",
    ),
