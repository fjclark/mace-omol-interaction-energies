...
for cutoff in "${CUTOFFS[@]}"; do
    ...
    python "${PYTHON_SCRIPT}"         --complex "${complex_pdb}"         --ligand "${ligand_sdf}"         --ligand-resnames "${LIGAND_RESNAMES}"         --ligand-charge "${LIGAND_CHARGE}"         --model "${MODEL}"         --device "${DEVICE}"         --output "${output_csv}"
done
