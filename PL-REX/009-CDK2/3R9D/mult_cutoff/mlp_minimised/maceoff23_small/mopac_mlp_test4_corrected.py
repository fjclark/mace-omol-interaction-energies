...
def ml_device_for_platform(platform_name: OpenMMPlatform) -> str:
    """Map an OpenMM platform name to the MACE/PyTorch device."""
    if platform_name in {"CPU", "Reference"}:
        return "cpu"
    return "cuda"


def build_mlp_system(
    molecule: Molecule,
    model: AvailableModel,
    ml_device: str,
) -> openmm.System:
    """Create a ligand-only OpenMM-ML system on the requested device."""
    if MLPotential is None:
        raise ImportError(
            "openmmml is required for ligand MLP optimisation: "
...
def select_openmm_platform(
    ...
    """Select an OpenMM platform and only attach GPU properties to GPU platforms."""
    available = {
...
def create_simulation(
    ...
    """Create a Simulation without passing GPU properties to CPU/Reference."""
...
def run_mlp_on_sdf(pdb_id: str, source_type: str, sdf_path: Path, model: AvailableModel, auto_model_by_charge: bool, restraint_k: float, max_iterations: int, platform_name: OpenMMPlatform, minimised_sdf_out: Path) -> MLPResult:
    ...
    ml_device = ml_device_for_platform(platform_name)
    print(
        f"[{pdb_id}] Ligand-only MACE device: {ml_device}",
        flush=True,
    )
    system = build_mlp_system(mol, selected_model, ml_device)
...
def run_pocket_mlp_optimisation(
    ...
    ml_device = ml_device_for_platform(platform_name)
    print(
        f"[{pdb_id}] Pocket MACE device: {ml_device}",
        flush=True,
    )

    mm_system = create_mm_system_for_complex(
        pdb.topology,
        ligand_molecule,
        nonbonded_mode,
    )
    system = create_openmmml_mixed_system(
        pdb.topology,
        mm_system,
        selected_model,
        ml_atoms,
    ...
    try:
        ...
        print(
            f"[{pdb_id}] Mobile-region force RMS =  "
        ...
        # Create pocket index array. At a true 0 A cutoff this is empty,
        # because only the ligand is mobile and the whole receptor is frozen.
        ...
        print(
            f"[{pdb_id}] Final mixed-system energy = "
            f"{final_energy:.6f} kcal/mol",
            flush=True,
        )
        ...
        if pocket_indices.size > 0:
            ...
            print(
                ...
                f"{np.sqrt(np.mean(pocket_displacements_angstrom**2)):.6f} A",
                flush=True,
            )

            print(
                f"[{pdb_id}] Pocket maximum displacement = "
                f"{np.max(pocket_displacements_angstrom):.6f} A",
                flush=True,
            )
        else:
            print(
            ...
            print(
                f"[{pdb_id}] Pocket RMS displacement = N/A",
                flush=True,
            )
            print(
                f"[{pdb_id}] Pocket maximum displacement = N/A",
                flush=True,
            )
        ...
        print(
            ...
            f"{np.sqrt(np.mean(ligand_final_force_norms**2)):.6f} "
            f"kJ/mol/nm",
            flush=True,
        )
...
# Add a function for cutoff-specific filenames
# `:g` removes unnecessary trailing zeros
# e.g. str(5.0) will print 5.0 while f"(5.0:g)" will print 5
# .replace(".", "p") will replace every "," with "p"
def cutoff_output_label(cutoff_angstrom: float) -> str:
    """Return a filename-safe label such as cutoff_0A or cutoff_2p5A."""
    value = f"{cutoff_angstrom:g}".replace(".", "p")
    return f"{value}A"
...
def run_one_pdb(
    ...
    # for more than one pocket_cutoff
    pocket_cutoffs: list[float],
    pocket_mode: PocketMode,
    freeze_backbone: bool,
    nonbonded_mode: NonbondedMode,
) -> dict[str, Any]:
    ...
    if not skip_pocket_mlp:
        ...
        pocket_results: list[PocketMLPResult] = []

        # Each optimisation starts from the same original complex_path
        for cutoff in pocket_cutoffs:
            label = cutoff_output_label(cutoff)
            cutoff_dir = paths.results_dir / f"pocket_cutoff_{label}"

            output_pdb = cutoff_dir / "complex_pocket_mlp_minimised.pdb"
            output_ligand_sdf = cutoff_dir / "complex_pocket_mlp_minimised_ligand.sdf"
            output_json = cutoff_dir / "pocket_mlp_result.json"
            output_csv = cutoff_dir / "pocket_mlp_result.csv"

            print(
                f"[{pdb_id}] Running pocket cutoff {cutoff:g} A"
                f"-> {cutoff_dir}",
                flush=True
            )
            ...
            pocket_result = run_pocket_mlp_optimisation(
                ...
                output_ligand_sdf=output_ligand_sdf,
            )
            write_dataclass_result(output_json, output_csv, pocket_result)
            pocket_results.append(pocket_result)

            # Add cutoff-specific columns to the per-PDB batch summary row.
            prefix = f"pocket_{label}"
            row.update(
                {
                    f"{prefix}_pocket_mlp_status": pocket_result.status,
                    f"{prefix}_pocket_mlp_model": pocket_result.ml_model,
                    f"{prefix}_pocket_mlp_initial_energy": pocket_result.initial_energy_kcal_per_mol,
                    f"{prefix}_pocket_mlp_final_energy": pocket_result.final_energy_kcal_per_mol,
                    f"{prefix}_pocket_mlp_energy_delta": pocket_result.energy_delta_kcal_per_mol,
                    f"{prefix}_pocket_mlp_n_ml_atoms": pocket_result.n_ml_atoms,
                    f"{prefix}_pocket_mlp_n_mobile_atoms": pocket_result.n_mobile_atoms,
                    f"{prefix}_pocket_mlp_n_frozen_atoms": pocket_result.n_frozen_atoms,
                    f"{prefix}_pocket_mlp_pdb": pocket_result.output_pdb_path,
                    f"{prefix}_pocket_mlp_ligand_sdf": pocket_result.output_ligand_sdf_path,
                    f"{prefix}_pocket_ligand_input": str(bound_ligand_sdf),
                }
            )
        row["pocket_ligand_input"] = str(bound_ligand_sdf)
...
def main(
    ...
    model: AvailableModel = typer.Option("mace-off23-small", "--model", help="MLP model."),
    ...
    ligand_resname: str = typer.Option("UNL", "--ligand-resname", help="Ligand residue name in the complex PDB."),
    # pocket_cutoff: float = typer.Option(5.0, "--pocket-cutoff", min=0.0, help="Residue cutoff around ligand in Angstrom. Uses strict '< cutoff'."),
    pocket_cutoffs: list[float] = typer.Option(
        [0.0, 2.0, 5.0, 10.0, 20.0],
        "--pocket-cutoff",
        min=0.0,
        help=(
            "Repeatable residue cutoff around the ligand in Angstrom. "
            "Defaults to 0, 2, 5, 10 and 20 A; selection uses strict '< cutoff'. "
            "At 0 A, only the ligand is mobile and the receptor is frozen."
        ),
    ),
    ...
    freeze_backbone: bool = typer.Option(
        ...
        help="Kept for CLI compatibility. Pocket backbone is still flexible because ligand + pocket atoms are mobile.",
    ),
